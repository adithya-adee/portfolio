          ...
          <div className="flex flex-wrap gap-3 sm:gap-4">
            {navigationLinks.map((link) => (
              <Link
                key={link.name}
                href={link.url}
                className="flex items-center gap-2.5 rounded-lg border border-neutral-700/40 bg-neutral-900/50 px-4 py-3 text-sm tracking-wide text-gray-400 transition-all duration-300 hover:-translate-y-0.5 hover:border-neutral-600/60 hover:bg-neutral-800/50 hover:text-white hover:shadow-lg hover:shadow-purple-500/10"
              >
                {link.icon}
                <span>{link.name}</span>
              </Link>
